...
GLdouble maxXMandelbrot = 1.5f;
GLdouble minXMandelbrot = -1.5f;
...
void renderTribrot(void)
{
	...
	{                                                                                       // z = x + i*y;
		for (y = 0, z.y = minYMaldenbrot; y < currScreenHeight; y++, z.y += imaginaryYMandelbrot)
		{
			...
			{
				t.x = t2.x*t2.x*t2.x - 3 * t2.x*t2.y*t2.y + z.x;
				t2.y = 3 * t2.x*t2.x*t2.y - t2.y*t2.y*t2.y + z.y;
				t2.x = t.x;
