...
clock_t executionCodeStart;

GLdouble zoomIntensityJulia = 0.1;  // speed of the zoom in Julia set
GLdouble minXJulia = -1.5f;    // fractal bounds
GLdouble maxXJulia = 1.5f; // -----||-----
GLdouble minYJulia = -1.25f; // -----||-----
GLdouble maxYJulia = 1.25f; // -----||-----
GLdouble imaginaryXJulia;    // current bound X
GLdouble imaginaryYJulia;    // current bound Y
...
void juliaSpecKeyboardControl(int key, int x, int y)
{
	if (key == GLUT_KEY_UP)
	{
		maxYJulia += zoomIntensityJulia;
		minYJulia += zoomIntensityJulia;
	}
	...
	{
		maxXJulia += zoomIntensityJulia;
		minXJulia += zoomIntensityJulia;
	}
...
void juliaKeyboardControl(unsigned char key, int x, int y)
{
	...
	if (key == 'q' || key == 'Q')
	{
		if (maxXJulia - minXJulia>3 * zoomIntensityJulia)
		{
			maxXJulia -= zoomIntensityJulia;
			minXJulia += zoomIntensityJulia;
			maxYJulia -= zoomIntensityJulia * zoomCoef;
			minYJulia += zoomIntensityJulia * zoomCoef;
		}
		else
			zoomIntensityJulia = zoomIntensityJulia / 10;
	}
	if (key == 'a' || key == 'A')
	{
		maxXJulia += zoomIntensityJulia;
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Edgaro Žigio Julijos aibė");
	glutPositionWindow((width - 700) / 2, (height - 700) / 2);
	glutReshapeFunc(reshape);
	glutDisplayFunc(renderJulia);
	glutSpecialFunc(juliaSpecKeyboardControl);
	glutKeyboardFunc(juliaKeyboardControl);
	init();
	glutMainLoop();
